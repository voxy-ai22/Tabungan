...
import { Plus, Settings as SettingsIcon, Wallet, ArrowUpRight, ArrowDownLeft, Trash2, Moon, Sun, Bell, Image as ImageIcon, X, PieChart, CheckCircle2, TrendingUp, Download, Smartphone } from 'lucide-react';
      ...
      <AnimatePresence>
        ...
        {showInstallBanner && (
          <motion.div
            ...
            className="fixed bottom-6 left-4 right-4 z-[70] max-w-sm mx-auto"
          >
            <div className="bg-zinc-900 dark:bg-zinc-800 text-white rounded-3xl p-4 shadow-2xl flex items-center gap-4 border border-white/10">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-black text-sm">Install Celengan Digital</p>
                <p className="text-[11px] text-white/60">Akses cepat dari homescreen kamu!</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 bg-green-500 hover:bg-green-400 text-white text-xs font-black px-3 py-2 rounded-xl transition-colors active:scale-95"
                >
...
// Theme card preview metadata
const THEME_META: Record<ThemeType, { label: string; desc: string; preview: { bg: string; card: string; accent: string; text: string } }> = {
  brutalist: {
    label: 'Brutalist',
    desc: 'Bold & raw energy',
    preview: { bg: '#E4E3E0', card: '#ffffff', accent: '#00FF00', text: '#141414' }
  },
  glass: {
    label: 'Glassmorphism',
    desc: 'Frosted & elegant',
    preview: { bg: '#1a1a2e', card: 'rgba(255,255,255,0.1)', accent: '#60a5fa', text: '#ffffff' }
  },
  standard: {
    label: 'Standard',
    desc: 'Clean & minimal',
    preview: { bg: '#f4f4f5', card: '#ffffff', accent: '#3b82f6', text: '#09090b' }
  },
  retro: {
    label: 'Retro / Y2K',
    desc: 'Nostalgia vibes',
    preview: { bg: '#1e1b4b', card: '#c0c0c0', accent: '#00ffff', text: '#000000' }
  },
  bento: {
    label: 'Bento Grid',
    desc: 'Smooth & rounded',
    preview: { bg: '#fafafa', card: '#ffffff', accent: '#18181b', text: '#18181b' }
  }
};

function ThemePreviewCard({ themeKey, isActive, onClick }: { themeKey: ThemeType; isActive: boolean; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "relative w-full text-left rounded-2xl overflow-hidden transition-all duration-300 group",
        isActive
          ? "ring-2 ring-offset-2 ring-offset-transparent shadow-xl scale-[1.02]"
          : "opacity-75 hover:opacity-100 hover:scale-[1.01]"
      )}
      style={isActive ? { boxShadow: `0 0 0 2px ${p.accent}` } : {}}
    >
      {/* Mini preview */}
      <div className="h-24 relative overflow-hidden" style={{ background: p.bg }}>
        {/* Mini card */}
        <div
          className="absolute left-3 top-3 bottom-3 w-20 rounded-lg flex flex-col justify-between p-2"
          style={{
            background: themeKey === 'glass' ? 'rgba(255,255,255,0.12)' : p.card,
            border: themeKey === 'brutalist' ? `2px solid ${p.text}` : themeKey === 'retro' ? `2px solid white` : '1px solid rgba(0,0,0,0.08)',
            backdropFilter: themeKey === 'glass' ? 'blur(8px)' : undefined,
            boxShadow: themeKey === 'brutalist' ? `3px 3px 0px ${p.text}` : themeKey === 'retro' ? '2px 2px 0px #666' : undefined,
          }}
        >
          <div className="h-1.5 w-10 rounded-full" style={{ background: p.text, opacity: 0.7 }} />
          <div>
            <div className="h-1 w-14 rounded mb-1" style={{ background: p.text, opacity: 0.3 }} />
            <div className="h-2 rounded" style={{ background: p.accent }} />
          </div>
        </div>

        {/* Mini second element */}
        <div
          className="absolute right-3 top-4 w-14 h-6 rounded"
          style={{
            background: p.accent,
            border: themeKey === 'brutalist' ? `1.5px solid ${p.text}` : undefined,
            boxShadow: themeKey === 'brutalist' ? `2px 2px 0px ${p.text}` : undefined,
            borderRadius: themeKey === 'bento' ? '12px' : themeKey === 'brutalist' ? '0' : '6px',
            opacity: 0.9
          }}
        />
        <div
          className="absolute right-3 bottom-4 w-10 h-4 rounded opacity-50"
          style={{ background: p.text, borderRadius: themeKey === 'bento' ? '8px' : themeKey === 'brutalist' ? '0' : '4px' }}
        />
      ...
      <div className="px-3 py-2 bg-black/5 dark:bg-white/5 flex justify-between items-center">
        <div>
          <p className="text-xs font-black" style={{ color: isActive ? p.accent : undefined }}>{meta.label}</p>
          <p className="text-[10px] opacity-50">{meta.desc}</p>
        </div>
        {isActive && <div className="w-1.5 h-1.5 rounded-full" style={{ background: p.accent }} />}
      </div>
    </button>
  );
}
...
function SettingsModal({ settings, setSettings, onClose, onRequestNotify }: { 
  ...
  const [reminderTime, setReminderTime] = React.useState(settings.reminderTime || '20:00');
  ...
  const applyReminderTime = (time: string) => {
    ...
    setSettings((s: AppSettings) => ({ ...s, reminderTime: time, reminderActive: true }));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      <motion.div 
        ...
        className={cn(
          "w-full max-w-sm ml-auto relative z-10 flex flex-col overflow-hidden shadow-2xl",
          "bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100"
        )}
      >
        {/* Header */}
        <div className="flex justify-between items-center px-6 py-5 border-b border-zinc-200 dark:border-zinc-800 shrink-0">
          <div>
            <h2 className="text-2xl font-display font-black tracking-tighter">Pengaturan</h2>
            <p className="text-[10px] uppercase font-bold opacity-40 tracking-widest mt-0.5">Sesuaikan tampilanmu</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-[11px] uppercase font-black tracking-widest text-blue-500">Tema Visual</h3>
            </div>
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-[11px] uppercase font-black tracking-widest text-purple-500">Tampilan</h3>
            </div>
            <button 
              onClick={() => setSettings({...settings, mode: settings.mode === 'light' ? 'dark' : 'light'})}
              className="w-full flex items-center justify-between p-4 rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-purple-400 transition-all group"
            >
              <div className="flex items-center gap-3">
                <div className={cn(
                  "w-10 h-10 rounded-xl flex items-center justify-center transition-colors",
                  settings.mode === 'dark' ? "bg-indigo-950 text-yellow-300" : "bg-yellow-100 text-yellow-600"
                )}>
                  {settings.mode === 'light' ? <Sun size={20} /> : <Moon size={20} />}
                </div>
                <div className="text-left">
                  <p className="font-bold text-sm">{settings.mode === 'light' ? 'Mode Terang' : 'Mode Gelap'}</p>
                  <p className="text-[10px] opacity-50">{settings.mode === 'light' ? 'Nyaman di siang hari' : 'Hemat baterai & enak di malam hari'}</p>
                </div>
              </div>
              <div className={cn(
                "w-12 h-6 rounded-full relative transition-colors duration-300 shrink-0",
                settings.mode === 'dark' ? "bg-purple-600" : "bg-zinc-300"
              )}>
                <div className={cn(
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-[11px] uppercase font-black tracking-widest text-green-500">Notifikasi PWA</h3>
            </div>
            ...
            <div className="space-y-3">
              ...
              <button 
                ...
                className={cn(
                  ...
                  settings.notificationsEnabled
                    ? "bg-green-50 dark:bg-green-950/30 border-green-300 dark:border-green-800 cursor-default"
                    : "bg-zinc-100 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 hover:border-green-400"
                )}
              >
                <div className="flex items-center gap-3">
                  <div className={cn(
                    "w-10 h-10 rounded-xl flex items-center justify-center",
                    settings.notificationsEnabled ? "bg-green-500 text-white" : "bg-zinc-200 dark:bg-zinc-800"
                  )}>
                    <Bell size={18} />
                  </div>
                  <div className="text-left">
                    <p className="font-bold text-sm">{settings.notificationsEnabled ? 'Notifikasi Aktif' : 'Aktifkan Notifikasi'}</p>
                    <p className="text-[10px] opacity-50">{settings.notificationsEnabled ? 'Pengingat siap dikirim' : 'Tap untuk memberi izin'}</p>
                  </div>
                </div>
                <span className={cn(
                  "text-[10px] font-black px-2 py-1 rounded-full",
                  settings.notificationsEnabled ? "bg-green-500 text-white" : "bg-zinc-200 dark:bg-zinc-700"
                )}>
                  {settings.notificationsEnabled ? '✓ AKTIF' : 'IZINKAN'}
                </span>
              </button>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] opacity-50">Notifikasi otomatis setiap hari</p>
                      </div>
                    </div>
                    <div
                      onClick={() => setSettings({ ...settings, reminderActive: !settings.reminderActive })}
                      className={cn(
                        "w-12 h-6 rounded-full relative transition-colors duration-300 cursor-pointer shrink-0",
                        settings.reminderActive ? "bg-orange-500" : "bg-zinc-300"
                      )}
                    >
                      <div className={cn(
                        "absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all duration-300",
                        settings.reminderActive ? "right-1" : "left-1"
                      )} />
                    </div>
                  </div>

                  {settings.reminderActive && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-3 pt-2 border-t border-current/10">
                      <div>
                        <label className="text-[10px] uppercase font-black opacity-50 block mb-2">Jam Pengingat</label>
                        <div className="flex items-center gap-3">
                          <input
                            type="time"
                            value={reminderTime}
                            onChange={e => setReminderTime(e.target.value)}
                            className="flex-1 bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-xl px-3 py-2 text-sm font-bold focus:outline-none focus:border-orange-400"
                          />
                          <button
                            onClick={() => applyReminderTime(reminderTime)}
                            className="px-4 py-2 bg-orange-500 text-white rounded-xl text-xs font-black hover:bg-orange-600 transition-colors active:scale-95"
                          >
                            SIMPAN
                          </button>
                        </div>
              ...
              {settings.notificationsEnabled && (
                <button
                  ...
                  className={cn(
                    "w-full p-3 rounded-2xl border border-dashed transition-all text-center text-xs font-bold",
                    testSent
                      ? "border-green-400 text-green-500 bg-green-50 dark:bg-green-950/20"
                      : "border-zinc-300 dark:border-zinc-700 opacity-60 hover:opacity-100 hover:border-blue-400 hover:text-blue-500"
                  )}
                >
          ...
          <section>
            <div className="p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border border-blue-200 dark:border-blue-800">
              <p className="text-xs font-black text-blue-600 dark:text-blue-400 mb-1">💡 Install sebagai App</p>
              <p className="text-[10px] text-blue-500/70 dark:text-blue-400/60 leading-relaxed">
                Tambahkan ke homescreen agar bisa dibuka seperti app native, bahkan tanpa internet!
              </p>
            </div>
          </section>
        ...
        <div className="px-6 py-4 border-t border-zinc-200 dark:border-zinc-800 shrink-0 flex items-center justify-between">
          <p className="text-[10px] opacity-30 font-mono">CELENGAN DIGITAL V2.0</p>
          <button
            onClick={onClose}
